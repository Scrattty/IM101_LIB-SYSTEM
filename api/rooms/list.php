<?php
// Set headers for JSON response and CORS
header("Access-Control-Allow-Origin: *");
header("Content-Type: application/json; charset=UTF-8");
header("Access-Control-Allow-Methods: GET, OPTIONS");
header("Access-Control-Max-Age: 3600");
header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");

// Handle preflight OPTIONS request
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit();
}

// Include necessary files
include_once '../config/db.php';
include_once '../models/Room.php';

// Instantiate database and room objects
$database = new Database();
$db = $database->getConnection();

if (!$db) {
    http_response_code(503); // Service Unavailable
    echo json_encode(array("status" => "error", "message" => "Unable to connect to database."));
    exit;
}

$room = new Room($db);

// Attempt to get all rooms
try {
    $stmt = $room->getAllRooms();
    
    if ($stmt && $stmt->rowCount() > 0) {
        $rooms_arr = array();
        $rooms_arr["status"] = "success";
        $rooms_arr["rooms"] = array();
        
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            extract($row);
            
            $room_item = array(
                "room_id" => $room_id,
                "room_name" => $room_name,
                "room_type" => $room_type,
                "capacity" => $capacity,
                "description" => $description,
                "is_available" => $is_available
            );
            
            array_push($rooms_arr["rooms"], $room_item);
        }
        
        http_response_code(200);
        echo json_encode($rooms_arr);
    } else {
        http_response_code(404); // Not Found
        echo json_encode(array("status" => "error", "message" => "No rooms found or all rooms are unavailable."));
    }
} catch (Exception $e) {
    http_response_code(500); // Internal Server Error
    echo json_encode(array("status" => "error", "message" => "Error retrieving rooms: " . $e->getMessage()));
}
?> 